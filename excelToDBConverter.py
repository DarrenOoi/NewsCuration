...
def addCampaigns():
    df = pd.read_excel(os.getcwd() + "\\exploration\\campaignsData.xlsx", sheet_name='Politician_CampaignPolicies')
    df = df.reset_index()
    ...
    for index, row in df.iterrows():
        ...
        fname, lname, policyTitle, policyInfo = [str(row[value]).encode("ascii", errors="ignore").decode().replace("'", "") for value in ['Fname', 'Lname', 'PolicyNameTitle', 'PolicyInfo']]
